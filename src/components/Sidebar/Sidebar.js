...
export default function Sidebar({ routes, logo, dropdown, input }) {
  ...
  const onClickParent = (event) => {
    ...
    let name = event.target.attributes[1].value;
    let temp ='{"'+name+'":true}';
    temp = JSON.parse(temp)
    temp[name] = !openChildren[name];
    setOpenChildren({...openChildren, ...temp})
  }
  ...
  const createLinks = (routes) => {
    ...
    return routes.map((prop, key) => {
      ...
      if(prop.parentName) {
        return (
          <Collapse key={key} in={!!openChildren[prop.parentName]} timeout="auto" unmountOnExit style={{paddingLeft: "30px"}}>
            <ListItem
              component={Link}
